...
def plot_losses(losses, y_lim, title, file_name, xlabel='Epoch', ylabel='Loss', legend=None):
    ...
    for key in losses.keys():
        if legend is None:
            plt.plot(losses[key]['epoch'], losses[key]['loss'], label=str(key+8), color=colors[key % 8])
        else:
...
mainlosses = {}
mainlosses[0] = pd.read_csv('losses/loss_N70_batch32_lr0.001_fd4_b0.5.csv', sep=',', names=['epoch', 'pos_loss', 'vel_loss', 'loss'])
mainlosses[1] = pd.read_csv('losses/val_N70_batch32_lr0.001_fd4_b0.5.csv', sep=',', names=['epoch', 'pos_loss', 'vel_loss', 'loss'])
...
plot_losses(trainlosses, 0.3, 'Training losses', 'plots/training_losses_large.png')
plot_losses(trainlosses, 0.0015, 'Training losses', 'plots/training_losses_small.png')
plot_losses(vallosses, 0.1, 'Validation losses', 'plots/validation_losses_large.png')
plot_losses(vallosses, 0.004, 'Validation losses', 'plots/validation_losses_small.png')
plot_losses(vallosses, 0.0001, 'Validation losses', 'plots/validation_losses_extrasmall.png')

plot_losses(mainlosses, 0.01, 'Training and Validation losses', 'plots/main_losses_large.png', legend=['Training', 'Validation'])
plot_losses(mainlosses, 0.0005, 'Training and Validation losses', 'plots/main_losses_small.png', legend=['Training', 'Validation'])
